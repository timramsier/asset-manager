...
const AssetInputGroup = React.createClass({
  ...
  removeAsset(_id) {
    const newState = this.state;
    const index = newState.assets.findIndex(i => i._id === _id);
    if (index > -1) {
      newState.assets.splice(index, 1);
      if (index > -1) this.setState(newState);
    }
  },
  componentWillMount() {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (actions.remove.length > 0) {
      actions.remove.forEach(_id => {
        this.removeAsset(_id);
      });
    }
    if (actions.add.length > 0) {
      actions.add.forEach(_id => {
        api._get('assets', { _id }).then(asset => {
