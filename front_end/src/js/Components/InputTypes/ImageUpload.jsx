...
const InputImage = React.createClass({
  ...
  handleFileUpload(event) {
    ...
    if (file) {
      ...
      const imgTag = document.getElementById(`preview-${this._idName}`);
      reader.onload = e => {
        imgTag.style.backgroundImage = `url(${e.target.result})`;
      };
      image.onload = () => {
        ...
        if (formData._id && formData._id !== '') fileName = formData._id;
        const imageFileName = `${fileName}.${file.name.split('.').pop()}`;
        this.props.updateFormData({
          image: `/public/uploads/${imageFileName}`,
        });
  ...
  render() {
    ...
    return (
      <div className="image-upload">
        <div className="preview-image" id={`preview-${this._idName}`} />
        <FormControl
          id={this._idName}
          type="file"
          name="image"
        ...
        <div className="image-controls">
          <label
            ...
            htmlFor={this._idName}
          >
            <FontAwesome name="upload" /> {this.state.buttonText}
          </label>
          <Button
