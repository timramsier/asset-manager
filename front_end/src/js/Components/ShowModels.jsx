...
const ShowModels = React.createClass({
  ...
  pushData(data) {
    if (this._isMounted) {
      ...
      newState.models.push(...data);
      this.setState(newState);
    }
  },
  updateModelData(updateType = 'refresh') {
    let active;
    const category = this.props.match.params.productType.toLowerCase();
  ...
  updateHeaderAccentColor() {
    ...
    if (this.props.match.params.productType !== 'all') {
      color = this.props.categories.filter(
        category =>
          category.label === this.props.match.params.productType.toLowerCase()
      )[0].config.color;
    } else {
  ...
  componentDidMount() {
    ...
    window.addEventListener('scroll', () => {
      if (!this.state.loading && this.state.update) {
        ...
        if (last < 0) last = 0;
        const elems = document.querySelectorAll('.asset-card');
        const lastElem = elems[last];
  ...
  render() {
    ...
    const locationPath = (
      <span className="location-path">
        <a href="/">
          <FontAwesome className="fa-fw path-icon" name="home" />
        </a>{' '}
