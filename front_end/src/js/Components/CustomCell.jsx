...
const DateCell = React.createClass({
  ...
  render() {
    ...
    return <Cell {...dimensons}>{formattedDate}</Cell>;
  },
});
...
const TextCell = React.createClass({
  ...
  render() {
    ...
    const dimensons = { height, width };
    return <Cell {...dimensons}>{cellData}</Cell>;
  },
});
...
const RemoveCell = React.createClass({
  ...
  removeData(data) {
    ...
    const removeData = cellData =>
      new Promise((resolve, reject) => {
        const url = `${this.props.apiCall}/all/${cellData._shortId}`;
        api
          ._delete(url)
          .then(response => {
            if (response.status === 200) {
              this.props.flashMessage(
                'success',
                <span>
                  <strong>Success!</strong> <em>{cellData.name}</em>{' '}
    ...
    removeData(data).then(removeImage);
  },
  render() {
        ...
        this.props.openConfirmModal({
          ...
          body: (
            <span>
              ...
              <strong>{cellData.name || cellData.poNumber}</strong>?
            </span>
          ),
          onConfirm: () => {
    ...
    return (
      <Cell {...dimensons} className="admin-cell">
        <a
          title={`Remove ${cellData.name || cellData.poNumber || ''}`}
          {...handleClick}
        >
...
const EditCell = React.createClass({
  ...
  render() {
    ...
    return (
      <Cell {...dimensons} className="admin-cell">
        <a
          title={`Edit ${cellData.name || cellData.poNumber || ''}`}
          {...clickEffect}
        >
