      ...
      if (fileInputs[i].files[0]) {
        ...
        if (name === 'image') {
          if (component.props.data._id && component.props.data._id !== '') {
            fileName = `${component.props.data._id}.${fileName.split('.').pop()}`
          } else {
            fileName = `${component.state.tempId}.${fileName.split('.').pop()}`
          }
        }
